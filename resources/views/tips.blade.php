        ...
        <div class="rounded-3xl bg-[#fff8ee]/80 p-6 shadow-lg shadow-rose-200/30 sm:p-8">
            @auth
                ...
                <form method="POST" action="{{ route('account.tips.store') }}" class="mt-5 grid gap-4">
                    @csrf
                    <input name="title" type="text" required maxlength="120" placeholder="Tip title" class="rounded-xl border-rose-100 bg-white px-4 py-3 focus:border-rose-400 focus:ring-rose-400" value="{{ old('title') }}">
                    @error('title')
